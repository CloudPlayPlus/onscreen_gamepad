...
普通按键与视角 R3 支持文字或内置 `buttonIcon` 图案（JSON `ic`）：奔跑/行走、跳跃、下蹲、趴下、刀、盾牌、药瓶、菜单、背包、射击子弹、换弹子弹与回转箭头、瞄准。人物和动作朝左，射击为左上子弹，换弹缩小子弹配左下/右上弯箭头。图案使用相同前景透明度，占按钮视觉直径的 62%；奔跑/行走跟随实际按键持有状态。标签和输入绑定保持独立，选择图案不修改输入语义。
