...
- 右摇杆 `stickMode`（`sm`）可选默认 `joystick` 或 `camera`。后者只在半屏空白处按相邻触点差输出鼠标相对移动，按下不跳变、不输出摇杆向量；`mouseSensitivity`（`ms`）默认 1，范围 0.1–5。原摇杆本体始终作为 R3 按钮显示，按住/松开发 R3 down/up，拖动本体不转视角。R3 和区域视角可由两指同时操作；切后台、隐藏、卸载和配置变更释放 R3 并取消旧手势。
- 左摇杆 `autoRun`（`ar`）默认开启。上推方向在垂直向上 ±30° 时，半月上方出现奔跑图标与箭头；图标中心距半月中心为半径加 56 dp。松手时满力度且提示触点距图标中心不超过 30 dp 才锁定；原中心额外半月模式使用相对半月显示的小点判定，其他情况使用实际触点，取消事件不锁定。
