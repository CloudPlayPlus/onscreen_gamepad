...
- `floatingFollow` 摇杆：视觉尺寸不变，隐形触发区扩大为标准 hit rect 直径的 `1.8` 倍。按下点同时成为输入零点和视觉圆心；拖出最大输入半径后，输出保持满量程，圆心沿拖动方向跟随手指。激活时在底盘外沿绘制八个扇区，扇区边界在八向切换临界线处留缝，当前扇区高亮并延长两侧边界线，抬手后视觉圆心复位。
