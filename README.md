...
`buttonIcon`（JSON `ic`）默认 `text`，另有 81 个图标，分为八个 `OnscreenGamepadIconCategory` 类型。`icon.category`、`icon.label`、`icon.matches(query)` 供宿主选择器按分类、名称和别名搜索；模型仍只保存枚举名，不保存分类或搜索状态。内置轻量矢量图案继承前景透明度和尺寸，保持原文字名称与绑定。`runWalk` 在实际按住、锁定或滑动持有时显示奔跑，释放时显示行走。
...
普通按键（键盘键、鼠标键、手柄键及视角 R3）可开启 `mouseDrag`（JSON `md`，默认关闭），保持原按压模式和绑定。`dragOutput`（JSON `do`）选择 `rightStick`（默认）、`leftStick` 或 `mouse`。摇杆以按下点为中心，50 logical pixels 达满量程，偏移不变时保持向量，回中心或抬指归零；鼠标模式复用 `mouseSensitivity`（默认 10，范围 1–50）。拖出按钮仍有效，按键锁定不保持拖动输出。滑动连按只采用起手按钮的输出设置，R3 按钮和半屏视角触点独立记录位置。
